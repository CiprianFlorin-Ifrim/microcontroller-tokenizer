...
// Define the maximum size for the serial buffer and the total input size
#define MAX_INPUT_SIZE 2048  // Maximum size of the entire input string
...
void setup() {
  Serial.begin(2000000);  // Set baud rate to 2,000,000
  while (!Serial) {
...
void loop() {
  static char input[MAX_INPUT_SIZE] = {0};
  static int inputLength = 0;
  static unsigned long lastReadTime = millis();  // Initialize to prevent immediate processing

  while (Serial.available() > 0) {
    char receivedChar = Serial.read();

    // Store characters in buffer if there's space
    if (inputLength < MAX_INPUT_SIZE - 1) {
      input[inputLength++] = receivedChar;
      lastReadTime = millis();
    } else {
      // Once buffer is full, exit to prevent overflow
      Serial.println("Buffer full. Processing data.");
      break;
    }
  }

  // Check for timeout to process data (100ms since last character received)
  if (millis() - lastReadTime > 100 && inputLength > 0) {
    input[inputLength] = '\0';  // Ensure null-termination

    Serial.println("Processing input:");
    Serial.println(input);

    // Now, apply the Huffman encoding and decoding process
    String inputStr = String(input);  // Convert the char array to a String
    ...
    Serial.flush();  // Flush the serial to clean any residual data
  }
}
